...
import { CircleDollarSign, Sparkles, X, Loader2, ArrowLeft } from "lucide-react";
...
import { cacheTipCurrency, getCachedTipCurrency } from "../services/cache.service";
...
export const TipConfirmDialog = ({
  ...
  // DESO calculations
  const amountNanos = exchangeRate != null ? usdToNanos(activeAmount, exchangeRate) : 0;
  const desoBalanceUsd = exchangeRate != null ? nanosToUsd(appUser.BalanceNanos, exchangeRate) : null;
  const desoEquivalent = exchangeRate != null && activeAmount > 0
    ? (activeAmount * 100) / exchangeRate : 0;

  // USDC calculations
  const usdcBalanceUsd = usdcBalance != null ? usdcBaseUnitsToUsd(usdcBalance) : null;
  ...
  const canSend =
    ...
    (currency === "DESO"
      ...
      : usdcBalance != null && usdcBaseUnits <= usdcBalance && appUser.BalanceNanos >= MIN_DESO_FOR_FEES);

  const tier =
    activeAmount >= LARGE_THRESHOLD ? "large"
      : activeAmount >= MEDIUM_THRESHOLD ? "medium"
        : "small";

  // Color theme based on currency: DESO = blue, USDC = green
  const accentColor = currency === "DESO" ? "#2775ca" : "#34F080";
  const accentTextClass = currency === "DESO" ? "text-[#2775ca]" : "text-[#34F080]";
  const gradientClasses = currency === "DESO"
  ...
  useEffect(() => {
    ...
    fetchUsdcBalance(appUser.PublicKeyBase58Check).then(setUsdcBalance).catch(() => {});
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !sending) {
        if (showHeroSwap) { setShowHeroSwap(false); return; }
        onClose();
        return;
      }
  ...
  const executeTip = useCallback(async () => {
    ...
    try {
      ...
      if (currency === "DESO") {
        ...
        const hasPerms = identity.hasPermissions({
          GlobalDESOLimit: MIN_DESO_FOR_FEES,
      ...
      onTipSent({
        ...
        amountUsdcBaseUnits: currency === "USDC" ? toHexUint256(usdcBaseUnits) : undefined,
    ...
    canSend, currency, amountNanos, usdcBaseUnits,
    appUser.PublicKeyBase58Check, recipientPublicKey, recipientUsername,
    tipReplyTo, tipMessage, onTipSent, onClose,
  ]);
  ...
  const handleSendClick = useCallback(() => {
    ...
    if (tier === "small") executeTip();
      ...
      if (confirmArmed) executeTip();
        ...
        confirmTimerRef.current = setTimeout(() => setConfirmArmed(false), CONFIRM_TIMEOUT_MS);
      }
    } else if (tier === "large" && largeConfirmChecked) executeTip();
  }, [canSend, tier, confirmArmed, largeConfirmChecked, executeTip]);

  const displayName = recipientUsername ? `@${recipientUsername}` : `${recipientPublicKey.slice(0, 8)}...`;
  const afterTipColor =
    afterTipUsd == null ? "text-gray-400"
      : afterTipUsd > 1 ? accentTextClass
        : afterTipUsd > 0.1 ? "text-amber-400"
          : "text-red-400";

  const getButtonContent = () => {
    if (sending) return <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Sending...</span>;
    if (tier === "medium" && confirmArmed) return `Confirm ${formatUsd(activeAmount)}`;
    if (tier === "large" && !largeConfirmChecked) return `Check the box above to confirm`;
    return `Tip ${formatUsd(activeAmount)} to ${displayName}`;
  };

  const getButtonClasses = () => {
    const base = "w-full py-3 rounded-xl text-sm font-bold transition-all cursor-pointer relative overflow-hidden";
    if (!canSend || (tier === "large" && !largeConfirmChecked)) return `${base} bg-white/5 text-gray-500 cursor-not-allowed`;
    if (tier === "medium" && confirmArmed) return `${base} bg-amber-500/20 border border-amber-500/40 text-amber-300 hover:bg-amber-500/30`;
    return `${base} ${gradientClasses} hover:shadow-[0_0_30px_rgba(${currency === "DESO" ? "39,117,202" : "52,240,128"},0.3)] active:scale-[0.98]`;
  };
    ...
    return createPortal(
      <div className="fixed inset-0 z-[10000] bg-[#050e1d] flex flex-col pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
          ...
          }} className="text-gray-400 hover:text-white transition-colors cursor-pointer" aria-label="Go back">
  ...
  if (exchangeRate == null) {
    return createPortal(
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60">
        <div role="dialog" aria-modal="true" aria-label="Loading tip dialog"
  ...
  const currencyLabel = currency === "USDC" ? `${activeAmount.toFixed(2)} USDC` : `${desoEquivalent.toFixed(4)} DESO`;
  const balanceLabel = currency === "USDC"
    ? `${usdcBalanceUsd != null ? formatUsd(usdcBalanceUsd) : "..."} USDC`
    : `${desoBalanceUsd != null ? formatUsd(desoBalanceUsd) : "..."} (${desoNanosToDeso(appUser.BalanceNanos).toFixed(4)} DESO)`;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 modal-backdrop-enter" onClick={onClose}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={`Send tip to ${displayName}`}
        ...
        <div className={`h-1 ${currency === "DESO"
        ...
        {sent ? (
          <div className="p-8 text-center">
            <div className={`w-16 h-16 mx-auto mb-5 rounded-full border flex items-center justify-center`}
            ...
            <p className="text-gray-400 text-sm mb-1">
              You tipped <span className={`${accentTextClass} font-semibold`}>{formatUsd(activeAmount)}</span> to {displayName}
            </p>
            <p className="text-gray-600 text-xs mb-6">{currencyLabel} sent on-chain</p>
            <button onClick={onClose}
              className={`px-8 py-2.5 border font-semibold rounded-lg transition-colors cursor-pointer`}
          ...
          <>
            <div className="flex items-center justify-between px-5 pt-5 pb-3">
              <div className="flex items-center gap-2.5">
                <CircleDollarSign className="w-5 h-5" style={{ color: accentColor }} />
                <h3 className="text-lg font-bold text-white">Send Tip</h3>
              </div>
              <button onClick={onClose} aria-label="Close tip dialog"
            ...
            <div className="px-5 pb-5">
              ...
              <div className="flex items-center gap-3 mb-4">
                <MessagingDisplayAvatar publicKey={recipientPublicKey} username={recipientUsername} diameter={40} />
                <div>
              ...
              <div className="grid grid-cols-4 gap-2 mb-3">
                {PRESET_AMOUNTS_USD.map((amount) => {
                  ...
                  return (
                    <button key={amount} aria-label={`Tip ${formatUsd(amount)} ${currency}`}
                      ...
                      className={`py-2.5 rounded-lg text-sm font-semibold transition-all cursor-pointer ${
                        active ? `${gradientClasses} shadow-[0_0_20px_rgba(${currency === "DESO" ? "39,117,202" : "52,240,128"},0.2)]`
              ...
              {/* Custom amount */}
              <div className={`flex items-center gap-2 rounded-lg border px-3 py-2 mb-3 transition-colors ${
                isCustom
                  ? (currency === "DESO"
                ...
                <input type="number" inputMode="decimal" step="0.01" min="0.01"
                  aria-label="Custom tip amount in USD" placeholder="Custom amount"
                  value={customAmount} onFocus={() => setIsCustom(true)}
              ...
              {/* Currency equivalent */}
              {activeAmount > 0 && (
                <div className="text-xs text-gray-500 mb-3">≈ {currencyLabel}</div>
              )}

              {/* Optional message */}
              <textarea placeholder="Add a message (optional)" aria-label="Tip message"
                value={tipMessage} onChange={(e) => setTipMessage(e.target.value)} rows={2}
              ...
              <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                ...
                <span className="font-semibold text-gray-400">{balanceLabel}</span>
              </div>
              {activeAmount > 0 && afterTipUsd != null && (
                <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
                  <span>After tip</span>
                  <span className={`font-semibold ${afterTipColor}`}>{formatUsd(Math.max(0, afterTipUsd))}</span>
                </div>
              )}
              ...
              {needsDesoForFees && (
                <div className="text-xs text-amber-400/80 mb-3 text-center">
                  ...
                  <button onClick={() => { setHeroSwapForFees(true); setShowHeroSwap(true); }} className="text-[#3b82f6] underline cursor-pointer">Get DESO</button>
                </div>
              )}
              {activeAmount > 0 && balanceUsd != null && activeAmount > balanceUsd && !needsDesoForFees && (
                <div className="text-xs text-red-400/80 mb-3 text-center">
                  Insufficient balance.{" "}
                  <button onClick={() => setShowHeroSwap(true)} className="text-[#3b82f6] underline cursor-pointer">Add funds</button>
                </div>
              )}
              ...
              {tier === "large" && (
                <label className="flex items-start gap-2.5 mb-3 cursor-pointer select-none">
                  <input type="checkbox" checked={largeConfirmChecked}
                    onChange={(e) => setLargeConfirmChecked(e.target.checked)}
                    className="mt-0.5 w-4 h-4 accent-amber-500 shrink-0" />
                  <span className="text-xs text-amber-400/80">
                    I confirm I want to send <span className="font-semibold">{formatUsd(activeAmount)}</span> ({currencyLabel}) to {displayName}
                  </span>
                </label>
              )}
              ...
              <p className="text-[10px] text-gray-600 text-center mt-3">
                Sent directly on-chain via DeSo {currency === "USDC" ? "(USDC)" : ""}
              </p>
            </div>
          </>
        )}
