...
export const EmojiPickerButton = ({ onEmojiSelect }: EmojiPickerButtonProps) => {
  ...
  const mobileOverlay = open && isMobile && mobilePortalTarget ? createPortal(
    <div
      ref={mobilePickerRef}
    ...
    >
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-blue-800/30">
        ...
        <button onClick={() => setOpen(false)} aria-label="Close" className="cursor-pointer">
          ...
          <div className="grid grid-cols-8 gap-1 justify-items-center">
            {POPULAR_EMOJI.map((emoji) => (
              <button
                ...
                className="flex items-center justify-center w-10 h-10 rounded-md hover:bg-white/10 active:bg-white/15 cursor-pointer transition-colors"
              >
    ...
    <div className="relative" ref={containerRef}>
      <button
        onClick={() => {
          ...
          if (opening && isMobile) (document.activeElement as HTMLElement)?.blur();
        }}
